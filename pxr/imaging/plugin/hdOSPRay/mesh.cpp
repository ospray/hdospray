...
TF_DEFINE_PRIVATE_TOKENS(HdOSPRayTokens, (st)(ptexFaceOffset));
...
HdOSPRayMesh::HdOSPRayMesh(SdfPath const &id, SdfPath const &instancerId)
    : HdMesh(id, instancerId), _ospMesh(nullptr), _adjacencyValid(false),
      _normalsValid(false), _refined(false), _smoothNormals(false),
      _doubleSided(false), _cullStyle(HdCullStyleDontCare) {}

void HdOSPRayMesh::Finalize(HdRenderParam *renderParam) {
  _ospInstances.clear();
}

HdDirtyBits HdOSPRayMesh::GetInitialDirtyBitsMask() const {
  ...
  int mask =
      ...
      HdChangeTracker::AllDirty // this magic bit seems to trigger materials...
                                // bah
      ;
...
HdDirtyBits HdOSPRayMesh::_PropagateDirtyBits(HdDirtyBits bits) const {
  return bits;
}

void HdOSPRayMesh::_InitRepr(TfToken const &reprToken, HdDirtyBits *dirtyBits) {
  TF_UNUSED(dirtyBits);
...
void HdOSPRayMesh::Sync(HdSceneDelegate *sceneDelegate,
  ...
  // HdReprSelector calculatedReprToken = _GetReprSelector(reprToken,
  // forcedRepr);
  ...
  OSPModel model =
      static_cast<HdOSPRayRenderParam *>(renderParam)->GetOSPRayModel();
  OSPRenderer renderer =
      static_cast<HdOSPRayRenderParam *>(renderParam)->GetOSPRayRenderer();
...
void HdOSPRayMesh::_UpdatePrimvarSources(HdSceneDelegate *sceneDelegate,
...
void HdOSPRayMesh::_PopulateOSPMesh(HdSceneDelegate *sceneDelegate,
  ...
  if (newMesh ||
    ...
    if (useQuads) {
      ...
      // usd stores texcoords in face indexed -> each quad has 4 unique
      // texcoords.
      ...
      auto indices =
          ospNewData(_triangulatedIndices.size(), OSP_INT3,
    ...
    if (_computedNormals.size()) {
      auto normals =
          ospNewData(_computedNormals.size(), OSP_FLOAT3,
    ...
    if (_texcoords.size() > 1) {
      auto texcoords = ospNewData(_texcoords.size(), OSP_FLOAT2,
    ...
    ospAddGeometry(instanceModel, mesh); // crashing when added to the scene. I
                                         // suspect indices/vertex spec.
  ...
  if (!GetInstancerId().IsEmpty()) {
    ...
    VtMatrix4dArray transforms =
        static_cast<HdOSPRayInstancer *>(instancer)->ComputeInstanceTransforms(
            GetId());
...
void HdOSPRayMesh::_UpdateDrawItemGeometricShader(
    HdSceneDelegate *sceneDelegate, HdStDrawItem *drawItem,
    const HdMeshReprDesc &desc, size_t drawItemIdForDesc) {
  HdRenderIndex &renderIndex = sceneDelegate->GetRenderIndex();
