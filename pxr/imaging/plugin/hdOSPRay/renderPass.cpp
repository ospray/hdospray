...
HdOSPRayRenderPass::HdOSPRayRenderPass(HdRenderIndex *index,
    ...
    : HdRenderPass(index, collection), _pendingResetImage(false),
      _pendingModelUpdate(true), _renderer(renderer),
      _sceneVersion(sceneVersion), _lastRenderedVersion(0), _width(0),
      _height(0), _model(model), _inverseViewMatrix(1.0f) // == identity
      ,
  ...
  ospSet4f(_renderer, "bgColor", _clearColor[0], _clearColor[1], _clearColor[2],
           1.f);
...
void HdOSPRayRenderPass::_Execute(
  ...
  if (_width != vp[2] || _height != vp[3]) {
    _width = vp[2];
    _height = vp[3];
    _frameBuffer = ospNewFrameBuffer(osp::vec2i({(int)_width, (int)_height}),
                                     OSP_FB_RGBA32F,
  ...
  if (_useDenoiser && _numSamplesAccumulated >= _denoiserSPPThreshold) {
    int newSPP =
        std::max((int)HdOSPRayConfig::GetInstance().samplesPerFrame, 1) * 4;
    if (_spp != newSPP) {
      ospSet1i(_renderer, "spp", _spp);
      ospCommit(_renderer);
    }
